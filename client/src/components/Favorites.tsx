...
const FavoritesPage: React.FC = () => {
  ...
  const [favorites, setFavorites] = useState<INotes[]>([]);
    ...
    const getNotes = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:6972/notes/favorites`,
          {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setFavorites((prevNotes) => prevNotes.filter((note) => note.id !== id));
    } catch (error) {
  ...
  return (
    <div className='flex gap-6 justify-between flex-wrap mt-3 mx-auto max-w-2xl w-full'>
      <div className='flex gap-6 justify-between max-[704px]:justify-center flex-wrap z-30 overflow-y-auto max-h-[450px] w-full'>
        {favorites
          .filter((note) =>
